...
def visualize_images(dataset):
    if dataset == "CIFAR10":
        ...
        trainset = torchvision.datasets.Places365(
            root="./data", split="val", small=True, download=True
        )
...
def visualize_attention(model, dataset, output=None, device="cuda"):
    ...
    attention_maps = F.interpolate(
        attention_maps, size=image_size, mode="bilinear", align_corners=False
    )
    ...
    for i in range(num_images):
        ...
        extended_attention_map = np.concatenate(
            (np.zeros(image_size), attention_maps[i].cpu()), axis=1
        )
